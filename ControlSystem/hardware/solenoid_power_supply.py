...
class SolenoidPowerSupply(device.Device):
    ...
    comapre_actual_to_target()  = compare actual current to target current (should make an python error class for this!!)
    get(name)                   = gets the value of the target voltage and the actual current value
    set(name,value)             = sets the target value of "name" to "value"
    """
    ...
    def comapre_actual_to_target(self):
        if abs(self._get_actual_current() - self._target_current) > 1:
    ...
    def get(self, name):
        if name == 'voltage':
            ...
            return self._get_actual_current()
        else:
    ...
    def set(self, name, value):
        if name == 'voltage':
            ...
            if value != self._target_current:
                ...
                self.comapre_actual_to_target()
        else:
            raise ValueError('invalid attribute name (expected keyword "voltage" or "current")')
        if not self._connection.is_ok():
            self.set_error()
